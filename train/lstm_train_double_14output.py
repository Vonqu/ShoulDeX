...
for file in csv_files:
    ...
    expected_angle_cols = [f'angle{i}' for i in range(1, 15)]  # angle1 到 angle14
...
num_plots = 18 # 自动确定列数（你也可以手动指定列数）
cols = 3  # 例如一行放4个图，够紧凑
rows = math.ceil(num_plots / cols)
fig, axes = plt.subplots(rows, cols, figsize=(cols * 5, rows * 3))  # 动态尺寸
axes = axes.flatten()  # 转成一维方便访问
...
test_folder = './data\dft\qnc\mixed\\test'
...
for file in test_csv_files:
    ...
    #expected_sensor_cols = ['s1', 's2', 's3', 's4', 's5', 's6']
    #expected_angle_cols = [f'angle{i}' for i in range(1, 10)]
    expected_sensor_cols = [f's{i}' for i in range(1, 15)]
    #expected_angle_cols = [f'angle{i}' for i in range(1, 19)]
    expected_angle_cols = [f'angle{i}' for i in range(1, 19)]
...
# 加载训练好的 MultiHead 模型
model = MultiHeadLSTM(input_size, hidden_size=256, num_layers=3, dropout=0.1, output_size=output_size).to(device)
model.load_state_dict(torch.load(f"./model/lstm_model_db_100epoch{id_try}.pth"))
model.eval()
